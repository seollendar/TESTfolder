...
Adafruit_INA260 ina260_from_solar_panel   = Adafruit_INA260(); // 0x40
Adafruit_INA260 ina260_from_solar_charger = Adafruit_INA260(); // 0x41
Adafruit_INA260 ina260_to_load            = Adafruit_INA260(); // 0x44

float from_solar_panel_current  = 0.0;
float from_solar_panel_voltage  = 0.0;
float from_solar_panel_power    = 0.0;

float from_solar_charger_current  = 0.0;
float from_solar_charger_voltage  = 0.0;
float from_solar_charger_power    = 0.0;

float to_load_current = 0.0;
float to_load_voltage = 0.0;
float to_load_power   = 0.0;
...
//SERVER ADDRESS
IPAddress influxDB_Home(192,168,25,55);
IPAddress influxDB_Keti(203,253,128,166);
...
void setup() {
  ...
  if (!ina260_from_solar_panel.begin(0x40)) {
    Serial.println("Couldn't find INA260 chip - ina260_from_solar_panel");
    while (1);
  }

  if (!ina260_from_solar_charger.begin(0x41)) {
    Serial.println("Couldn't find INA260 chip - ina260_from_solar_charger");
    while (1);
  }

  if (!ina260_to_load.begin(0x44)) {
    Serial.println("Couldn't find INA260 chip - ina260_to_load");
    while (1);
  }
  ...
  display.print("Server: ");display.println(influxDB_Home);
  display.println("Wait for WiFi conn.");
  display.setCursor(0,0);
