...
if (cluster.isMaster) {
   ...
   * config setting
   */
const influxServer = `http://${env.influx.admin}:${env.influx.password}@${env.influx.host}:${env.influx.port}/${env.influx.database}`
const influx = new Influx.InfluxDB(influxServer);
...
app.listen(env.server.PORT, () => {
   console.log("API-Server Start on port " +env.server.PORT);
});
...
app.get("/location/:deviceID/:containerName/around", (req, res) => {
   ...
   if (
      ...
      // res.send([
      //    { lat: 37.103, lng: 127.159 },
      //    { lat: 37.11, lng: 127.128 }, 
      //    { lat: 37.118, lng: 127.127 }, 
      //    { lat: 37.127, lng: 127.144 } 
      //  ])
      let { deviceID, containerName } = req.params;
      ...
      client
         ...
         .then((response) => {
            if (response.rowCount) {
               ...
               // res.send({ devices });
               res.send([
                  { lat: 37.203, lng: 127.159 },
                  { lat: 37.21, lng: 127.128 }, 
                  { lat: 37.218, lng: 127.127 }, 
                  { lat: 37.227, lng: 127.144 } 
                ])
      ...
      res.status(400).send("Bad Request");
      console.log(req.params.deviceID ,req.params.containerName ,req.query.radius ,req.query.term);
      console.log("input value error");
   }
});
...
app.get("*", function (req, res) {
   res.status(404).send("Bad Request (Wrong Url)");
});
...
app.all("/*", function (req, res, next) {
   res.header(
      ...
      "X-Requested-With, Authorization, Content-Type, X-Platform, X-Version","Access-Control-Allow-Headers","Access-Control-Allow-Methods","Access-Control-Expose-Headers","Access-Control-Allow-Origin"
      );
      res.header(
