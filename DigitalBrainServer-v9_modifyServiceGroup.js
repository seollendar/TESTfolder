...
function create_flink_DO_table(DOobject) {
   ...
   // Create DO Table
   let createStreamSQL = {
      statement: `CREATE TABLE ${DOName}(\`sensorname\` STRING,\`con\` STRING, \`rowtime\` TIMESTAMP(3)) WITH ('connector' = 'kafka', 'topic' = 'DO_${DOName}','properties.bootstrap.servers' = '${config.kafkaHosts}', 'format'='json')`,
   };
   ...
   let insertTableSQL = {
      statement: `INSERT INTO ${DOName}(sensorname, con, rowtime)`,
   };

   sensorList.forEach((sensor, index) => {
      if (index === 0) {
         insertTableSQL.statement += ` SELECT '${sensor}' AS sensorname, con, rowtime FROM ${sensor}`;
      } else {
         insertTableSQL.statement += ` UNION ALL SELECT '${sensor}' AS sensorname, con, rowtime FROM ${sensor}`;
      }
   });
